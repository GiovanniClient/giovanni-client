...
public class GiovanniWorldRenderEvent {
    ...
    public GiovanniWorldRenderEvent(CameraRenderState camera, PoseStack matrices, DeltaTracker tickCounter, SubmitNodeCollector collector) {
        this.camera = camera;
    ...
    private void drawRat(RatReplacer.RatRenderData rat) {
        GlbRatModel.submit(collector, matrices, rat, camera.pos);
    }

    private void drawFilled(AABB box, int color) {
        collector.submitCustomGeometry(matrices, RenderTypes.debugFilledBox(),
                (pose, consumer) -> drawFilled(box, consumer, color));
    }

    private void drawFilled(AABB box, VertexConsumer consumer, int color) {
    ...
    private void drawOutline(AABB box, int color) {
        collector.submitCustomGeometry(matrices, RenderTypes.lines(),
                (pose, consumer) -> drawOutline(box, consumer, color));
    }
    ...
    private void drawPlayerTracers(float tickProgress) {
        ...
        Vec3 tracerStart = camera.pos.add(Vec3.directionFromRotation(
                ...
                client.gameRenderer.mainCamera().yRot()
        ).scale(10));
        for (AbstractClientPlayer player : client.level.players()) {
            if (player == client.player || !PlayerTracer.matches(player)) continue;

            AABB rawBox = getLerpedBox(player, tickProgress);
            AABB outline = rawBox.move(0, 0.05, 0).inflate(0.05);
            collector.submitCustomGeometry(matrices, GiovanniRenderLayers.TRACER_LINES, (pose, consumer) -> {
                drawOutline(outline, consumer, color);
